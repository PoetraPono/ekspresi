...
(function() {
	...
	function getInvalidHandler(invalidCallback) {
		return function(event) {
			...
			if (suffix && (specificErrormessage = element.getAttribute("eror-" + suffix))) {
				element.setCustomValidity(specificErrormessage);
			} else if (genericErrormessage = element.getAttribute("eror")) {
	...
	var civem = function() {
		...
		for (i = 0; i < formElements.length; i++) {
			...
			if (formElement.type.toUpperCase() === "RADIO" || formElement.type.toUpperCase() === "CHECKBOX") {
